...
impl Calculator {
    ...
    pub fn show_history(&self) -> String{
        ...
        for (i,op) in self.history.iter().enumerate() {
            if let Some(res) = op.operation_type.perform(op.first_num, op.second_num) {
                output.push_str(&format!("{}: {} {} {} = {}\n", i, op.first_num, op.operation_type.get_sign(), op.second_num, res));
            }
            else {
                output.push_str(&format!("{}: {} {} {} = overflow\n", i, op.first_num, op.operation_type.get_sign(), op.second_num));
            }
        }
        output
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        let op = self.history.get(operation_index)?;
